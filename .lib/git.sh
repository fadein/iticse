...
# Read-only return codes used by _tutr_git_file_status
...
_tutr_git_dir_status() {
	(( $# != 1 )) && _tutr_die echo _tutr_git_dir_status takes a directory name
    ...
    # Somehow, the curly brace makes this OK.
    # It worked in Zsh without this chicanery.
    git status --ignored --untracked=all --porcelain=v1 | {
        while IFS=$'\n' read line; do
            [[ $line != *" "$1/* ]] && continue  # l13
            case $line in
                "?? $1"*)         (( stat |= _GF_UNTRACKED )) ;;
...
STATETEXT
	...
	# Cannot inspect the .ls-remote file file with this function, as it's
	#	generated in the _test, which runs *after* this. To add any information
	#	from the .ls-remote file, one would need to modify the _test function
